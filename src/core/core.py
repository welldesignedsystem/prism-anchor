...
if __name__ == "__main__":
    ...
    # Minimal concrete stub — replace with real step implementations
    class NoOpStep(StepBase):
        def _run(self, context: WorkflowContext) -> None:
            context.set_result(self.name, f"{self.name} ran OK")

        def _validate(self, context: WorkflowContext) -> None:
            result = context.get_result(self.name)
            if not result:
                raise AssertionError(f"{self.name} produced no result")
    ...
    orchestrator = (
        WorkflowOrchestrator(ctx)
        .add_step(NoOpStep("S1_Setup"))
        .add_step(NoOpStep("S2_Audit"))
        .add_step(NoOpStep("S3_Monitor"))
        .add_step(NoOpStep("S4_Scores"))
        .add_step(NoOpStep("S5_Optimise"))
        .add_step(NoOpStep("S6_Alerts"))
        .add_step(NoOpStep("S7_Report"))
    )

    orchestrator.run_loop(max_iterations=2)

    print("\nStatus report:")
    for name, status in orchestrator.status_report().items():
